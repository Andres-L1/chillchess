...
<div class="w-full flex items-center gap-2 text-xs text-slate-400 font-mono">
    ...
    <div class="relative flex-1 h-3 bg-white/5 rounded-full group cursor-pointer flex items-center">
        <input
            type="range"
            min="0"
            ...
            aria-valuemin="0"
            aria-valuemax={Math.floor($audioStore.duration || 100)}
            aria-valuenow={Math.floor($audioStore.currentTime)}
            aria-valuetext="{formatTime($audioStore.currentTime)} de {formatTime(
                $audioStore.duration
            )}"
        />
        <!-- Background Track -->
        <div class="absolute w-full h-1 bg-white/10 rounded-full pointer-events-none"></div>
        ...
        <div
            ...
            style="width: {($audioStore.currentTime / ($audioStore.duration || 1)) * 100}%"
        >
