...
<div class="flex items-center gap-6 mb-1">
    <button
        ...
        aria-label={$audioStore.shuffle ? 'Desactivar aleatorio' : 'Activar aleatorio'}
        aria-pressed={$audioStore.shuffle}
        title="Aleatorio"
    >
    ...
    <button
        ...
        aria-pressed={$audioStore.isPlaying}
        title={$audioStore.isPlaying ? 'Pausar (Espacio)' : 'Reproducir (Espacio)'}
    >
    ...
    <button
        ...
        class="transition-colors relative {$audioStore.repeatMode !== 'off'
            ? 'text-green-400'
            : 'text-slate-400 hover:text-white'}"
        aria-label={$audioStore.repeatMode === 'one'
            ? 'Repetir una canción'
            : $audioStore.repeatMode === 'all'
              ? 'Repetir todas'
              : 'Repetir desactivado'}
        aria-pressed={$audioStore.repeatMode !== 'off'}
    ...
    >
        ...
        {#if $audioStore.repeatMode === 'one'}
            <span class="absolute -bottom-1 -right-1 text-[8px] font-bold" aria-hidden="true"
                >1</span
            >
        {/if}
    </button>
