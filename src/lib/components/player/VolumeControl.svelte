...
>
    ...
    <button
        ...
        aria-label={$audioStore.musicVolume === 0 ? 'Activar sonido' : 'Silenciar'}
    ...
    >
        <div class="relative w-full h-1.5 bg-white/10 rounded-full group/slider">
            <!-- Track Fill -->
            <div
            ...
            <input
                ...
                aria-valuemin="0"
                aria-valuemax="100"
                aria-valuenow={Math.round($audioStore.musicVolume * 100)}
                aria-valuetext="{Math.round($audioStore.musicVolume * 100)}%"
            />
        </div>
    </div>
</div>
