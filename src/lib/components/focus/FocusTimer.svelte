                ...
                <div class="flex-1 overflow-y-auto p-6 md:p-8 custom-scrollbar bg-[#0B1120]">
                    {#if vibeTab === 'scenes'}
                        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                            <!-- Replaced Emojis with Lucide-style SVGs (Simulated/Generic for now) -->
                            {#each [{ id: 'none', label: 'Silencio Digital', desc: 'Sin efectos', pro: false }, { id: 'noir', label: 'Lluvia Nocturna', desc: 'Lluvia suave y tonos oscuros', pro: false }, { id: 'library', label: 'Biblioteca', desc: 'Ambiente académico', pro: false }, { id: 'zen', label: 'Jardín Zen', desc: 'Naturaleza y viento', pro: true }, { id: 'space', label: 'Cosmos', desc: 'Frecuencias espaciales', pro: true }, { id: 'cyber', label: 'Cyber Grid', desc: 'Neón retro-futurista', pro: true }, { id: 'breathe', label: 'Modo Respiración', desc: 'Guía 4-7-8 pulsante', pro: true }] as vibe}
                                ...
                                >
                                    <div
                                        class="w-12 h-12 rounded-lg flex items-center justify-center text-white bg-white/5 border border-white/10 group-hover:scale-110 transition-transform"
                                    >
                                        <!-- Placeholder Icon -->
