...
<div class="min-h-screen bg-[#0B1120] text-slate-200 font-poppins overflow-x-hidden">
    ...
    <nav
        class="sticky top-0 z-50 w-full bg-[#0B1120]/80 backdrop-blur-xl border-b border-white/5 transition-all duration-300"
    >
        <div class="flex justify-between items-center px-4 md:px-8 py-4 max-w-7xl mx-auto relative">
            <!-- Mobile Hamburger (Left on Mobile) -->
            <button
                on:click={() => (mobileMenuOpen = !mobileMenuOpen)}
                class="md:hidden z-20 text-white p-2 hover:bg-white/10 rounded-lg transition-colors order-1"
                aria-label="Menu"
            >
                {#if mobileMenuOpen}
            ...
            <div
                class="flex items-center gap-3 z-20 order-2 md:order-1 absolute left-1/2 -translate-x-1/2 md:static md:translate-x-0 md:mr-auto"
            >
                <img
                ...
                <span
                    class="hidden md:inline-block text-xs uppercase bg-white/10 px-2 py-1 rounded-md text-slate-400"
                    >Beta v0.3</span
                >
            </div>
            ...
            <div
                class="hidden md:flex gap-6 text-sm font-medium text-slate-400 items-center order-3 ml-auto"
            >
                <a href="/roadmap" class="hover:text-primary-400 transition-colors">Roadmap</a>

                {#if $userStore.isLoggedIn}
                    <!-- ... Desktop Links ... -->
                    <a href="/coleccion" class="hover:text-primary-400 transition-colors"
                        >Colección</a
                    >
                ...
                {#if $userStore.isLoggedIn}
                    <div class="relative ml-4">
                        ...
                        {#if showUserMenu}
                            ...
                            >
                                <!-- Reusing same links as mobile -->
                                <a
                                    href="/artist"
                                    class="px-4 py-2 text-slate-300 hover:text-white hover:bg-white/5 text-left flex items-center gap-2"
                                >
                                    <UserIcon size="sm" gradient={false} /> <span>Mi Perfil</span>
                                </a>
                                <a
                                    href="/creators"
                                    class="px-4 py-2 text-slate-300 hover:text-white hover:bg-white/5 text-left flex items-center gap-2"
                                >
                                    <span class="text-sm">✨</span> <span>Creators Catalog</span>
                                </a>
                                <a
                                    href="/favorites"
                                    class="px-4 py-2 text-slate-300 hover:text-white hover:bg-white/5 text-left flex items-center gap-2"
            ...
            <!-- Mobile Profile Button (Right on Mobile) -->
            <div class="md:hidden z-20 order-3 flex items-center">
                {#if $userStore.isLoggedIn}
                    <div class="relative">
                        <button
                            on:click={() => (showUserMenu = !showUserMenu)}
                            class="w-10 h-10 rounded-full bg-gradient-to-tr from-orange-500 to-amber-500 p-[2px] shadow-lg"
                        >
                            <div
                                class="w-full h-full rounded-full bg-[#0B1120] flex items-center justify-center overflow-hidden"
                            >
                ...
                {:else}
                    <button
                        on:click={openAuth}
                        class="text-sm font-bold text-white bg-white/10 px-3 py-1.5 rounded-full"
                        >Acceso</button
                    >
                {/if}
            </div>
            ...
            {#if mobileMenuOpen}
                <div
                    class="absolute top-full left-0 right-0 mt-2 mx-4 bg-[#0B1120] border border-white/10 rounded-2xl shadow-2xl overflow-hidden md:hidden z-50 transform origin-top animate-fade-in"
                >
    ...
    <section
        class="max-w-7xl mx-auto px-4 md:px-8 py-16 relative z-10 border-b border-white/5 mb-20"
    >
        <div class="text-center mb-12">
            <span class="text-xs font-bold uppercase tracking-widest text-[#9146FF] mb-2 block"
                >Simple y Rápido</span
            >
        ...
        <div class="grid grid-cols-1 md:grid-cols-3 gap-8 relative">
            ...
            <div
                class="hidden md:block absolute top-[2.5rem] left-[16%] right-[16%] h-px bg-gradient-to-r from-transparent via-white/10 to-transparent -z-10"
            ></div>

            <!-- Step 1 -->
            <div class="flex flex-col items-center text-center group">
                <div
                    class="w-20 h-20 rounded-2xl bg-[#131b2e] border border-white/10 flex items-center justify-center text-white mb-6 shadow-lg shadow-blue-500/5 group-hover:scale-110 group-hover:border-blue-500/30 transition-all duration-300 relative z-10"
                >
                    <UserIcon size="lg" gradient={true} />
                    <div
                        class="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-blue-600 border-4 border-[#0B1120] flex items-center justify-center font-bold text-sm"
                    >
                        1
                    </div>
                </div>
                <h3 class="text-xl font-bold text-white mb-2">Crea tu Cuenta</h3>
                <p class="text-sm text-slate-400 max-w-[250px]">
                    Acceso instantáneo y gratuito. Sin tarjetas de crédito.
                </p>
            </div>
            ...
            <div class="flex flex-col items-center text-center group">
                <div
                    class="w-20 h-20 rounded-2xl bg-[#131b2e] border border-white/10 flex items-center justify-center text-white mb-6 shadow-lg shadow-purple-500/5 group-hover:scale-110 group-hover:border-purple-500/30 transition-all duration-300 relative z-10"
                >
                    <StarIcon size="lg" gradient={true} />
                    <div
                        class="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-purple-600 border-4 border-[#0B1120] flex items-center justify-center font-bold text-sm"
                    >
                        2
                    </div>
                </div>
                ...
                <p class="text-sm text-slate-400 max-w-[250px]">
                    Lo-Fi, Jazz, Synthwave... Personaliza tu entorno sonoro.
                </p>
            </div>
            ...
            <div class="flex flex-col items-center text-center group">
                <div
                    class="w-20 h-20 rounded-2xl bg-[#131b2e] border border-white/10 flex items-center justify-center text-white mb-6 shadow-lg shadow-green-500/5 group-hover:scale-110 group-hover:border-green-500/30 transition-all duration-300 relative z-10"
                >
                    <BoltIcon size="lg" gradient={true} />
                    <div
                        class="absolute -top-3 -right-3 w-8 h-8 rounded-full bg-green-600 border-4 border-[#0B1120] flex items-center justify-center font-bold text-sm"
                    >
                        3
                    </div>
                </div>
                ...
                <p class="text-sm text-slate-400 max-w-[250px]">
                    Dale al play y olvídate. Sin interrupciones ni anuncios.
                </p>
            </div>
        </div>
    </section>
