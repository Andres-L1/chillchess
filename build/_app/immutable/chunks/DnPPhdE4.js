 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const ds=new ao("@firebase/auth");function Lp(n,...e){ds.logLevel<=W.WARN&&ds.warn(`Auth (${Wt}): ${n}`,...e)}function ns(n,...e){ds.logLevel<=W.ERROR&&ds.error(`Auth (${Wt}): ${n}`,...e)}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function hl(n,e=[]){let t;switch(n){case"Browser":t=Sc(Re());break;case"Worker":t=`${Sc(Re())}-${n}`;break;default:t=n}const r=e.length?e.join(","):"FirebaseCore-web";return`${t}/JsCore/${Wt}/${r}`}/**
 * @license
 * Copyright 2022 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function gl(n,e,t=!1){if(Fe(n.app))return Promise.reject(tt(n));const r="signIn",s=await ml(n,r,e),i=await Bt._fromIdTokenResponse(n,r,s);return t||await n._updateCurrentUser(i.user),i}async function km(n,e){return gl(St(n),e)}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */async function _l(n){const e=St(n);e._getPasswordPolicyInternal()&&await e._updatePasswordPolicy()}async function _T(n,e,t){const r=St(n);await ps(r,{requestType:"PASSWORD_RESET",email:e,clientType:"CLIENT_TYPE_WEB"},"getOobCode",Tm)}async function yT(n,e,t){if(Fe(n.app))return Promise.reject(tt(n));const r=St(n),a=await ps(r,{returnSecureToken:!0,email:e,password:t,clientType:"CLIENT_TYPE_WEB"},"signUpPassword",bm).catch(l=>{throw l.code==="auth/password-does-not-meet-requirements"&&_l(n),l}),c=await Bt._fromIdTokenResponse(r,"signIn",a);return await r._updateCurrentUser(c.user),c}function vT(n,e,t){return Fe(n.app)?Promise.reject(tt(n)):km(Y(n),vn.credential(e,t)).catch(async r=>{throw r.code==="auth/password-does-not-meet-requirements"&&_l(n),r})}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const qt=new ao("@firebase/firestore");function jn(){return qt.logLevel}function O(n,...e){if(qt.logLevel<=W.DEBUG){const t=e.map(vo);qt.debug(`Firestore (${En}): ${n}`,...t)}}function it(n,...e){if(qt.logLevel<=W.ERROR){const t=e.map(vo);qt.error(`Firestore (${En}): ${n}`,...t)}}function ln(n,...e){if(qt.logLevel<=W.WARN){const t=e.map(vo);qt.warn(`Firestore (${En}): ${n}`,...t)}}function vo(n){if(typeof n=="string")return n;try{/**
* @license
* Copyright 2020 Google LLC
*
* Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class by{constructor(){this.Gi=!1,this.zi=!1,this.ji=100,this.Hi=function(){return pf()?8:o_(Re())>0?6:4}()}initialize(e,t){this.Ji=e,this.indexManager=t,this.Gi=!0}getDocumentsMatchingQuery(e,t,r,s){const i={result:null};return this.Yi(e,t).next(a=>{i.result=a}).next(()=>{if(!i.result)return this.Zi(e,t,s,r).next(a=>{i.result=a})}).next(()=>{if(i.result)return;const a=new Py;return this.Xi(e,t,a).next(c=>{if(i.result=c,this.zi)return this.es(e,t,a,c.size)})}).next(()=>i.result)}es(e,t,r,s){return r.documentReadCount<this.ji?(jn()<=W.DEBUG&&O("QueryEngine","SDK will not create cache indexes for query:",en(t),"since it only creates cache indexes for collection contains","more than or equal to",this.ji,"documents"),S.resolve()):(jn()<=W.DEBUG&&O("QueryEngine","Query:",en(t),"scans",r.documentReadCount,"local documents and returns",s,"documents as results."),r.documentReadCount>this.Hi*s?(jn()<=W.DEBUG&&O("QueryEngine","The SDK decides to create cache indexes for query:",en(t),"as using cache indexes may help improve performance."),this.indexManager.createTargetIndexes(e,ze(t))):S.resolve())}Yi(e,t){if(eu(t))return S.resolve(null);let r=ze(t);return this.indexManager.getIndexType(e,r).next(s=>s===0?null:(t.limit!==null&&s===1&&(t=Es(t,null,"F"),r=ze(t)),this.indexManager.getDocumentsMatchingTarget(e,r).next(i=>{const a=z(...i);return this.Ji.getDocuments(e,a).next(c=>this.indexManager.getMinOffset(e,r).next(l=>{const d=this.ts(t,c);return this.ns(t,d,a,l.readTime)?this.Yi(e,Es(t,null,"F")):this.rs(e,d,t,l)}))})))}Zi(e,t,r,s){return eu(t)||s.isEqual(B.min())?S.resolve(null):this.Ji.getDocuments(e,r).next(i=>{const a=this.ts(t,i);return this.ns(t,a,r,s)?S.resolve(null):(jn()<=W.DEBUG&&O("QueryEngine","Re-using previous result from %s to execute query: %s",s.toString(),en(t)),this.rs(e,a,t,t_(s,-1)).next(c=>c))})}ts(e,t){let r=new pe(Ql(e));return t.forEach((s,i)=>{Vs(e,i)&&(r=r.add(i))}),r}ns(e,t,r,s){if(e.limit===null)return!1;if(r.size!==t.size)return!0;const i=e.limitType==="F"?t.last():t.first();return!!i&&(i.hasPendingWrites||i.version.compareTo(s)>0)}Xi(e,t,r){return jn()<=W.DEBUG&&O("QueryEngine","Using full collection scan to execute query:",en(t)),this.Ji.getDocumentsMatchingQuery(e,t,Rt.min(),r)}rs(e,t,r,s){return this.Ji.getDocumentsMatchingQuery(e,r,s).next(i=>(t.forEach(a=>{i=i.insert(a.key,a)}),i))}}/**
 * @license
 * Copyright 2020 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */function Gs(n,e,t){let r=e;return t==null&&(r=`https://${e}`),`${t}://${r}/v0${n}`}function rd(n){const e=encodeURIComponent;let t="?";for(const r in n)if(n.hasOwnProperty(r)){const s=e(r)+"="+e(n[r]);t=t+s+"&"}return t=t.slice(0,-1),t}var Ut;(function(n){n[n.NO_ERROR=0]="NO_ERROR",n[n.NETWORK_ERROR=1]="NETWORK_ERROR",n[n.ABORT=2]="ABORT"})(Ut||(Ut={}));/**
 * @license
 * Copyright 2022 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */class zt{constructor(e,t){this._service=e,t instanceof Ve?this._location=t:this._location=Ve.makeFromUrl(t,e.host)}toString(){return"gs://"+this._location.bucket+"/"+this._location.path}_newRef(e,t){return new zt(e,t)}get root(){const e=new Ve(this._location.bucket,"");return this._newRef(this._service,e)}get bucket(){return this._location.bucket}get fullPath(){return this._location.path}get name(){return cd(this._location.path)}get storage(){return this._service}get parent(){const e=qE(this._location.path);if(e===null)return null;const t=new Ve(this._location.bucket,e);return new zt(this._service,t)}_throwIfRoot(e){if(this._location.path==="")throw vE(e)}}function sT(n,e,t){n._throwIfRoot("uploadBytes");const r=tT(n.storage,n._location,ud(),new vt(e,!0),t);return n.storage.makeRequestWithTokens(r,ha).then(s=>({metadata:s,ref:n}))}function iT(n){n._throwIfRoot("getDownloadURL");const e=XE(n.storage,n._location,ud());return n.storage.makeRequestWithTokens(e,ha).then(t=>{if(t===null)throw _E();return t})}function oT(n){n._throwIfRoot("deleteObject");const e=JE(n.storage,n._location);return n.storage.makeRequestWithTokens(e,ha)}function aT(n,e){const t=jE(n._location.path,e),r=new Ve(n._location.bucket,t);return new zt(n.storage,r)}/**
 * @license
 * Copyright 2017 Google LLC
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 ...
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */const md="storage";function WT(n,e,t){return n=Y(n),sT(n,e,t)}function HT(n){return n=Y(n),iT(n)}function KT(n){return n=Y(n),oT(n)}function GT(n,e){return n=Y(n),lT(n,e)}function dT(n=mr(),e){n=Y(n);const r=pr(n,md).getImmediate({identifier:e}),s=oo("storage");return s&&fT(r,...s),r}function fT(n,e,t,r={}){hT(n,e,t,r)}function pT(n,{instanceIdentifier:e}){const t=n.getProvider("app").getImmediate(),r=n.getProvider("auth-internal"),s=n.getProvider("app-check-internal");return new da(t,r,s,e,Wt)}function mT(){At(new nt(md,pT,"PUBLIC").setMultipleInstances(!0)),Oe(Ou,Lu,""),Oe(Ou,Lu,"esm2017")}mT();const gT={apiKey:"AIzaSyDkAPVdrwASXA-O5ajBU7T14qbKSfef5EI",authDomain:"chillchess-57365.firebaseapp.com",projectId:"chillchess-57365",storageBucket:"chillchess-57365.firebasestorage.app",messagingSenderId:"676151034372",appId:"1:676151034372:web:4124fbdfd7fee5dfee2b51",measurementId:"G-32YHTXR687"};let Mt,so,gd,io;try{Mt=Ip().length?mr():Wu(gT),so=zg(Mt),gd=Lv(Mt),io=rE(Mt),typeof window<"u"&&(window.location.hostname==="localhost"||window.location.hostname==="127.0.0.1")&&Zh(io,"localhost",5001),Dm(so,El).catch(n=>{console.warn("Auth Persistence Error:",n)})}catch(n){console.error("Error initializing Firebase (Check .env variables):",n)}let _d;try{Mt&&(_d=dT(Mt))}catch(n){console.warn("Storage init failed:",n)}const QT=Object.freeze(Object.defineProperty({__proto__:null,get app(){return Mt},get auth(){return so},get db(){return gd},get functions(){return io},get storage(){return _d}},Symbol.toStringTag,{value:"Module"}));export{Qn as $,ET as A,vT as B,_T as C,UT as D,FT as E,KT as F,Wv as G,xv as H,Uv as I,yn as J,wt as K,Ae as L,Qh as M,Pr as N,Gt as O,Ue as P,D as Q,Ho as R,ct as S,ce as T,ra as U,na as V,ls as W,Hs as X,ia as Y,sa as Z,Yh as _,so as a,Ko as a0,Qv as a1,Ml as a2,me as a3,ar as a4,L as a5,Hg as a6,fe as a7,we as a8,ln as a9,Nv as aa,Vv as ab,Hv as ac,An as ad,Cr as ae,Gv as af,Lv as ag,IT as ah,QT as ai,Ov as b,PT as c,gd as d,ST as e,DT as f,NT as g,_d as h,WT as i,HT as j,MT as k,BT as l,LT as m,xT as n,TT as o,CT as p,bT as q,GT as r,VT as s,kT as t,OT as u,jT as v,zT as w,$T as x,qT as y,yT as z};
